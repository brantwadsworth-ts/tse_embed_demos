// Ask Acme — the custom Acme AI experience (moved here from the
// Analytics "ACME AI" modal). Left: the Spotter answer canvas with a
// branded empty-state landing (logo + worksheet sample questions) and its own
// input bar hidden. Right: a "Acme AI" expert pane that tracks the
// question history and takes follow-ups.
//
// Each question is routed (via routeMessage): general Acme questions
...
import {
  WORKSHEET_ID,
  SPOTTER_EMBED_FLAGS,
  ACME_SAMPLE_QUESTIONS,
  ACME_TRIAL_QUESTIONS,
  ACME_VIDEO_URL,
...
const WELCOME: Turn = {
  role: 'assistant',
  text: 'Ask me anything about Acme — what it does, cadences, docs, best practices — or ask about your own data (“revenue by week”, “top cadences by influenced pipeline”) and I’ll chart it live.',
};
...
export default function AskAcme() {
  ...
  const runSpotterQuery = useCallback(
    (query: string) => {
      try {
        ...
        console.warn('[acme-ai] SpotterSearch failed:', e);
  ...
  async function send(preset?: string) {
    ...
    try {
      ...
      if (result.kind === 'analytics') {
        ...
        // General Acme question → answered from the knowledge base; Spotter
  ...
  const remaining = Math.max(0, ACME_TRIAL_QUESTIONS - askedCount);
      ...
      <div className="sl-ai-page">
          ...
          <div className={`sl-ai-canvas${spotterActive ? ' is-spotter-active' : ''}`}>
            {!currentQuery && (
              <div className="sl-ai-empty">
                <AcmeLogo className="sl-ai-empty-logo" size={58} wordmark={false} />
                <span className="sl-ai-empty-eyebrow">
                  <Sparkles size={14} /> Powered by Acme AI
                </span>
                <h2 className="sl-ai-empty-title">Ask Acme AI</h2>
        ...
        <div className="sl-ai-pane">
          ...
          <div className="sl-ai-pane-footer">
            <a href="https://www.acme.com/legal/privacy-notice" target="_blank" rel="noopener noreferrer">
              Privacy Policy
            </a>
