...
const ROLE_OPTIONS = ["admin", "create", "view"] as const;
...
export default function TeamManager() {
  ...
  async function handleRoleChange(login: string, newRole: Role) {
    setUpdatingRole(login);
    try {
        ...
        setData((prev) => {
          ...
          return {
            ...
            members: prev.members.map((m) =>
              m.login === login ? { ...m, role: newRole } : m
            ),
          };
        });
      }
  ...
  async function handleRemoveRole(login: string) {
    if (!confirm(`Remove role for @${login}? They will fall back to view-only access.`)) return;
          ...
          <div className="flex-1">
            <div className="flex gap-2">
              ...
              <select
                ...
                disabled={inviting}
                className="rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700 focus:border-[#2770ef] focus:outline-none focus:ring-2 focus:ring-[#2770ef]/20"
              >
                {ROLE_OPTIONS.map((r) => (
                ...
                {data.members.map((m) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        value={m.role}
                        onChange={(e) => handleRoleChange(m.login, e.target.value as Role)}
                        disabled={updatingRole === m.login}
                        className="rounded-lg border border-gray-200 px-2 py-1 text-xs text-gray-700 focus:border-[#2770ef] focus:outline-none focus:ring-2 focus:ring-[#2770ef]/20 disabled:opacity-50"
                      >
                        {ROLE_OPTIONS.map((r) => (
                          <option key={r} value={r}>{ROLE_LABELS[r]}</option>
                        ))}
                      ...
                      <button
                        onClick={() => handleRemoveRole(m.login)}
